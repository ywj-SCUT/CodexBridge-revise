...
async function runWeixinServe(args: string[]) {
  ...
  const mobileFilePicker = parseBooleanEnv(process.env.CODEXBRIDGE_MOBILE_UPLOAD_ENABLE, false)
    ? new MobileFilePickerService({
      ...
      miniProgramUrlTemplate: normalizeCliString(process.env.CODEXBRIDGE_MOBILE_UPLOAD_MINIPROGRAM_URL_TEMPLATE),
      miniProgramAppId: normalizeCliString(process.env.CODEXBRIDGE_MOBILE_UPLOAD_MINIPROGRAM_APP_ID),
      miniProgramAppSecret: normalizeCliString(process.env.CODEXBRIDGE_MOBILE_UPLOAD_MINIPROGRAM_APP_SECRET),
